...
/// Process a single batch by checking its status and generating appropriate events.
///
/// This is called by both the polling loop and the OpenAI webhook handler.
/// It fetches the batch status from OpenAI and processes the result through
/// the state machine.
